...
__global__	void gpu_simple_filter(unsigned char * image_origininal, unsigned char * image_result, unsigned int width, unsigned int height, int division_coef)
{
	...
	image_result[current_height*width + current_width] = (
		(
...
Result perform_GPU_simple_worker(Task task)
{
	...
	gpu_simple_filter << <grid, block >> > (image_original, image_result, task.work_matrix.width, task.work_matrix.height, task.division_coef);
	...
	cuda_status = cudaMemcpy(task.work_matrix.matrix,
		image_result,
		(task.work_matrix.height) * (task.work_matrix.width) *
